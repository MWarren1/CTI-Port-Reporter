...
## CLI switches
parser = argparse.ArgumentParser(prog='cti_reporter.py', description='Reports on the utilization of a sinlge gateway for every second for the length of the cdr file')
...
parsedoutputfile = "Parsed_CDR_CTI.csv"
outputfile = "Gateway_Report_CTI.csv"
...
with open(args.input, 'Ur') as f:
	...
	for row in parserreader:
		if row[56].find("CTI_") != -1 or row[57].find("CTI_") != -1:
